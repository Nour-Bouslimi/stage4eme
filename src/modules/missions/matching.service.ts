...
export class MatchingService {
  ...
  async findCandidates(options: {
    ...
    const candidates = all.filter((u) => {
      ...
      if (
        options.typeVehiculeRequis &&
        u.typeVehicule &&
        u.typeVehicule !== options.typeVehiculeRequis
      )
        return false;
      if (
        options.poidsEstime &&
        u.poidsMaxKg &&
        Number(u.poidsMaxKg) < options.poidsEstime
      )
        return false;
      if (
        options.volumeEstime &&
        u.volumeMaxM3 &&
        Number(u.volumeMaxM3) < options.volumeEstime
      )
        return false;
      if (u.latitudeActuelle == null || u.longitudeActuelle == null)
        return false;
      const dist = this.haversineDistance(
        options.latitudeRamassage,
        options.longitudeRamassage,
        Number(u.latitudeActuelle),
        Number(u.longitudeActuelle),
      );
      if (u.rayonServiceKm != null && dist > Number(u.rayonServiceKm))
        return false;
      return true;
    });
    return candidates;
